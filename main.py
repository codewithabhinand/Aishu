...
while True:
    ...
    reply = ChatAI(messages=messages)
    ...
    if re.search("agent_call",reply_contet):
        ...
        print(f"AI: {str(reply_contet).replace('user_call','')}")
    else:
        print(f"AI: {reply_contet}")
    
    msg = {"time":f'{datetime.now().strftime("%d-%m-%Y-%H:%M:%S")}',"user":f"{user_input}","aishu":f"{reply_contet}"}
